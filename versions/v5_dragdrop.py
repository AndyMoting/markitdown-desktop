"""v5_dragdrop.py — MarkItDown GUI v5
v4 基础上 + 拖拽文件到窗口（tkinterdnd2）
"""
import base64
import os
import re
...
class MarkItDownApp:
    def __init__(self, root: TkinterDnD.Tk):
        ...
        self.root.title("MarkItDown Converter — v5 Drag & Drop")
    ...
    def _setup_drag_drop(self):
        """用 tkinterdnd2 实现拖拽：注册 root 和 listbox 为 drop target。"""
        # 让 root 也接受拖放（listbox 外的区域也能拖）
        self.root.drop_target_register("*")
        self.root.dnd_bind("<<Drop>>", self._on_drop)

    def _on_drop(self, event):
        """处理拖放事件。event.data 是空格分隔、花括号括起路径的字符串。"""
        raw = event.data
