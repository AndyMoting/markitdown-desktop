...
def _init_logging(debug: bool):
    ...
    log_name = f"v{VERSION.replace('.', '_')}_{Path(sys.argv[0]).stem}.log"
...
def _extract_images(markdown: str, images_dir: str) -> tuple[str, int]:
    """把 base64 图片存到 images_dir, 返回 (新的 markdown, 图片数)。"""
    ...
    parts: list[str] = []
    pos = 0
    for match in _DATA_URI_RE.finditer(markdown):
        parts.append(markdown[pos : match.start()])
        pos = match.end()
        ...
        try:
            ...
            skipped += 1
            parts.append(f"![{alt}](broken:{filename})")
            continue

        if not decoded:
            _LOG.warning("零字节图片, 跳过 %s", filename)
            skipped += 1
            parts.append(f"![{alt}](empty:{filename})")
            continue
        ...
        parts.append(f"![{alt}](images/{filename})")

    parts.append(markdown[pos:])
    result = "".join(parts)
...
def _convert_one(filepath: str, save_to_tool: bool) -> ConvertResult:
    """转换单个文件, 返回 ConvertResult。"""
    ...
    # ---- .doc: 先转成 .docx, 再走 markitdown 的 DOCX 管线 ----
    ...
    if ext == ".doc":
        ...
        if docx_temp is None:
            return ConvertResult(False, ".doc → .docx 转换失败", 0, None)
    ...
    try:
        ...
        _LOG.error("markitdown 转换失败: %s — %s", filepath, e)
        return ConvertResult(False, str(e), 0, None)
    finally:
        if docx_temp:
            try:
                os.remove(docx_temp)
...
class MarkItDownApp:
    ...
    def _convert_thread(self, file_paths: list[str], save_to_tool: bool):
        ...
        for i, fp in enumerate(file_paths):
            ...
            t0 = time.time()
            try:
                result = _convert_one(fp, save_to_tool)
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("""\
            示例:
              ...
              markitdown-gui --debug                    启动界面并开启调试日志
              markitdown-gui --convert a.pdf b.doc      命令行模式
            """),
    )
