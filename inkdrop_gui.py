...
def _convert_one(filepath: str, save_to_tool: bool,
    ...
    # ---- 调用 markitdown ----
    text, img_count, call_error, image_paths = _call_markitdown(source_path, images_dir, md)
    ...
    if ext == ".pdf" and _config_get("pymupdf_accepted") is True:
        text, img_count, warning, image_paths = _postprocess_pdf(
    ...
    return ConvertResult(True, folder, img_count, warning,
                         images=image_paths, markdown=text)
...
def _call_markitdown(source_path: str, images_dir: str,
    ...
    """调用 markitdown 转换并提取图片。返回 (text, img_count, error_or_None, image_paths)。"""
    if md is None:
        md = MarkItDown()
    try:
        ...
        return text, img_count, None, image_paths
...
def _postprocess_pdf(filepath: str, text: str, img_count: int,
                     images_dir: str) -> tuple[str, int, str | None, list[str]]:
    """PDF 后处理: 双引擎文本对比 + 嵌入图片提取 + 输出量检查。
    返回 (text, img_count, warning, image_paths)."""
    ...
    if fitz_doc is None:
        warning = _check_output_short(filepath, text)
        return text, img_count, warning, _collect_image_paths(images_dir)
    ...
    image_paths = _collect_image_paths(images_dir)
    return text, img_count, warning, image_paths
...
class MarkItDownApp:
    ...
    def _build_image_gallery(self):
        self._gallery_frame = ttk.LabelFrame(self.root, text="图片预览")
        self._gallery_images: list[str] = []
        self._gallery_photos: list = []

        self._gallery_canvas = tk.Canvas(self._gallery_frame, height=90)
        self._gallery_scroll = ttk.Scrollbar(
            self._gallery_frame, orient=tk.HORIZONTAL,
            command=self._gallery_canvas.xview,
        )
        self._gallery_canvas.config(xscrollcommand=self._gallery_scroll.set)

        self._gallery_scroll.pack(side=tk.BOTTOM, fill=tk.X)
    ...
    def _update_image_gallery(self, images: list[str]):
        ...
        for idx, img_path in enumerate(images):
            ...
            frame = ttk.Frame(self._gallery_inner)
    ...
    def _open_image_viewer(self, index: int = 0):
        ...
        ttk.Label(dialog, textvariable=info_var, font=("", 9)).pack()

        nav_frame = ttk.Frame(dialog)
        nav_frame.pack(pady=6)

        prev_btn = ttk.Button(nav_frame, text="◀ 上一张")
        prev_btn.pack(side=tk.LEFT, padx=4)

        next_btn = ttk.Button(nav_frame, text="下一张 ▶")
        next_btn.pack(side=tk.LEFT, padx=4)

        ttk.Button(nav_frame, text="关闭", command=dialog.destroy,
        ...
        prev_btn.config(command=lambda: show(current[0] - 1))
        next_btn.config(command=lambda: show(current[0] + 1))
