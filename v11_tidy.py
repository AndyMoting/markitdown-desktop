r"""MarkItDown GUI v11 — 文档转 Markdown 桌面工具

功能: 拖拽/批量文档 → Markdown + 图片提取 → 输出文件夹
支持: PDF/DOCX/PPTX/XLSX/XLS/HTML/CSV/JSON/XML/TXT/图片/邮件/ZIP
入口: python v11_tidy.py [--help|--version|--debug|--convert FILE...]

=== 代码导航 ===
  1. config      配置读写 (%APPDATA%\markitdown-gui\)
  2. logging     日志 (WARNING+ 写文件, --debug 看全量)
  3. pdf_engine  PDF 引擎薄接口 (pdf_engine.py)
  4. 编码检测    乱码检测 + 双引擎对比
  5. 图片提取    base64 图片 (PDF 内嵌图片走 pdf_engine)
...
# ---- magika→filetype shim (saves ~42 MB in PyInstaller builds) ----
import sniffer
sys.modules["magika"] = sniffer
# Prevent the real magika deps from being pulled in
...
def _setup_logging(debug: bool):
    ...
    if not _log.handlers:
        ...
        fh = logging.FileHandler(
            _LOG_DIR / f"v11_{Path(sys.argv[0]).stem}.log",
            encoding="utf-8",
        )
...
# ============================================================
#  3. pdf_engine  PDF 引擎薄接口 — 见 pdf_engine.py
# ============================================================
...
class MarkItDownApp:
    ...
    def _on_close(self):
        ...
        if self.running and hasattr(self, '_thread') and self._thread.is_alive():
        ...
        for i, fp in enumerate(file_paths):
            if self.cancel_event.is_set():
                ...
                self.root.after(0, self._append_result,
                                f"已取消 ({completed}/{total})", "summary")
                self.root.after(0, self._on_all_done, completed, total, cancelled=True)
                return
...
def _show_pymupdf_dialog(args):
    ...
    if choice:
        ...
        pdf_engine.is_available()  # trigger lazy import
    else:
...
def _parse_args(argv: list[str] | None = None):
    ...
    parser.add_argument("--reset", action="store_true",
                        help="重置所有设置 (config、PyMuPDF 选择、调试日志)")
