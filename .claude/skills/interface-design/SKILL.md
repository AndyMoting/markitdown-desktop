...
> 来源: [Dammyjay93/interface-design](https://github.com/Dammyjay93/interface-design) (MIT)，原文收录。
>
> **本项目适配说明 (doc2md, tkinter 桌面应用):**
> - 实现介质是 tkinter 而非 HTML/CSS——"token" 对应 `doc2md/gui.py` 的 `COLORS` 字典，"component" 对应构建方法；CSS/动效/Radix 等 Web 专属条目按精神执行（状态完备、层级、克制），不逐字照搬。
> - 设计决策沉淀到 `.interface-design/system.md`（按本 skill 的 After Completing a Task 节）。
> - 验证方式：PrintWindow 截图（scratchpad 的 gui_screenshot.py 模式），非浏览器。
